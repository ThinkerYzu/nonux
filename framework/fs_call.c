/*
 * Slice 9b.1 — FS sync-dispatcher wrappers (ID-based API).
 *
 ...
 *
 * Slice 9b.1 changes the API: `open` now returns a uint32_t component-
 * local ID (0 = failure); close/retain/read/write/seek take `uint32_t id`
 * instead of `void *file`.  No kernel pointers cross the IPC boundary.
 *
 ...
 * satisfying the 8.0e iface_ops rule.
 *
...
uint32_t nx_fs_open(struct nx_slot *slot, const char *path, uint32_t flags)
{
#if __STDC_HOSTED__
    HOST_RESOLVE_FS(slot, ops, 0);
    if (!ops->open) return 0;
    return ops->open(slot->active->impl, path, flags);
#else
    int (*hmsg)(void *, struct nx_ipc_message *);
    void *impl;
    if (disp_resolve(slot, &hmsg, &impl) != NX_OK) return 0;
...
void nx_fs_close(struct nx_slot *slot, uint32_t id)
{
#if __STDC_HOSTED__
...
void nx_fs_retain(struct nx_slot *slot, uint32_t id)
{
#if __STDC_HOSTED__
    HOST_RESOLVE_FS_VOID(slot, ops);
...
int64_t nx_fs_read(struct nx_slot *slot, uint32_t id, void *buf, size_t cap)
{
#if __STDC_HOSTED__
...
int64_t nx_fs_write(struct nx_slot *slot, uint32_t id, const void *buf,
                    size_t len)
{
#if __STDC_HOSTED__
    HOST_RESOLVE_FS(slot, ops, (int64_t)NX_ENOENT);
...
int64_t nx_fs_seek(struct nx_slot *slot, uint32_t id, int64_t offset,
                   int whence)
{
#if __STDC_HOSTED__
    HOST_RESOLVE_FS(slot, ops, (int64_t)NX_ENOENT);
