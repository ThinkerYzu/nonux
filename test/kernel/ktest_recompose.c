...
/* ---- Per-test fixture ------------------------------------------------- *
 *
 * Slots are module-level static (see TESTING-GUIDE §"Kernel test fixture
 * pattern"): the slot-nodes in g_slots always point to valid memory.
 * Components are per-test (stack-allocated in KTEST body via fixture struct)
 * and unregistered in fixture_down while still in scope.
 *
 * nx_slot_unregister is NOT called — static slots stay in g_slots.
 * nx_dispatcher_reset is NOT called — dropping messages without decrementing
 * in_flight_calls would leave scheduler.in_flight_calls > 0 and hang the
 * live_swap drain loop.
 */

static struct nx_slot s_krecomp_src;
static struct nx_slot s_krecomp_tgt;
                                ...
                                enum nx_pause_policy policy)
{
    ...
    s_krecomp_src = (struct nx_slot){ .name = src_name, .iface = "krecomp_test",
                                      .mutability  = NX_MUT_HOT,
                                      .concurrency = NX_CONC_SHARED };
    s_krecomp_tgt = (struct nx_slot){ .name = tgt_name, .iface = "krecomp_test",
                                      .mutability  = NX_MUT_HOT,
                                      .concurrency = NX_CONC_SHARED };
    ...
    (void)nx_slot_register(&s_krecomp_src);
    (void)nx_slot_register(&s_krecomp_tgt);
    (void)nx_component_register(&f->src_comp);
    (void)nx_component_register(&f->old_comp);
    (void)nx_component_register(&f->new_comp);
    (void)nx_slot_swap(&s_krecomp_src, &f->src_comp);
    (void)nx_slot_swap(&s_krecomp_tgt, &f->old_comp);
    ...
    int err = NX_OK;
    f->edge = nx_connection_register(&s_krecomp_src, &s_krecomp_tgt,
...
static void krecomp_fixture_down(struct recomp_fixture *f)
{
    ...
    (void)nx_slot_swap(&s_krecomp_src, NULL);
    (void)nx_slot_swap(&s_krecomp_tgt, NULL);
    (void)nx_component_unregister(&f->src_comp);
    /* nx_recompose calls nx_component_destroy (→ DESTROYED) but not
     * nx_component_unregister; remove the node explicitly so the DESTROYED
     * guard in component_node_find doesn't have to paper over it. */
...
KTEST(recompose_kernel_leaf_slot_swap)
{
    ...
    struct nx_slot_change change = { .slot = &s_krecomp_tgt, .action = NX_SLOT_REPLACE,
                                     .new_comp = &f.new_comp };
...
KTEST(recompose_kernel_hold_queue_reaches_new_component)
{
    ...
    KASSERT_EQ_U(nx_slot_pause_state(&s_krecomp_tgt), NX_SLOT_PAUSE_DONE);

    /* Send two messages — they land on the hold queue (QUEUE policy). */
    struct nx_ipc_message m1 = { .src_slot = &s_krecomp_src, .dst_slot = &s_krecomp_tgt, .msg_type = 1 };
    struct nx_ipc_message m2 = { .src_slot = &s_krecomp_src, .dst_slot = &s_krecomp_tgt, .msg_type = 2 };
    KASSERT_EQ_U(nx_ipc_send(&m1), NX_OK);
    KASSERT_EQ_U(nx_ipc_send(&m2), NX_OK);
    KASSERT_EQ_U(nx_ipc_hold_queue_depth(&s_krecomp_src, &s_krecomp_tgt), 2);
    ...
    KASSERT_EQ_U(nx_ipc_hold_queue_depth(&s_krecomp_src, &s_krecomp_tgt), 0);
