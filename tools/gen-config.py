...
def load_kernel(path: pathlib.Path) -> dict:
    ...
    for slot in sorted(components.keys()):
        ...
        if not isinstance(alternatives, list):
            raise ManifestError(
                f"components.{slot!r}.alternatives: expected list")
        for alt in alternatives:
            if not isinstance(alt, str) or not alt:
                raise ManifestError(
                    f"components.{slot!r}.alternatives: entries must be "
                    f"non-empty strings")
            c_ident(alt)
...
def render_sources_mk(kernel: dict, kernel_path: pathlib.Path) -> str:
    ...
    slots bound to the same impl produce one source entry.  Alternative
    implementations listed under a slot's "alternatives" key are also
    compiled in so they are available for live swap at runtime.
    """
    impl_set: set[str] = set()
    for b in kernel["bindings"]:
        impl_set.add(b["impl"])
        for alt in b.get("alternatives", []):
            impl_set.add(alt)
    srcs = sorted(impl_set)
    lines: list[str] = []
    lines.append(
        f"# Auto-generated by tools/gen-config.py from "
        f"{kernel_path.name}. Do not edit.")
